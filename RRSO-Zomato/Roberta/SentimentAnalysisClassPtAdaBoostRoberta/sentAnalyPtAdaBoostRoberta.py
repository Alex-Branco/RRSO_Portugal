...
# https://huggingface.co/thegoodfellas/tgf-xlm-roberta-base-pt-br
...
save_name = 'SentAnalyPtAdaBoostRoberta_' + str(NumberIterations) # change the name to avoid overwrite
...
path = './NewRoberta/SentimentAnalysisClassPtAdaBoostRoberta/'
...
# Load Tokenizer:
# tokenizer = AutoTokenizer.from_pretrained("thegoodfellas/tgf-xlm-roberta-base-pt-br")
tokenizer = AutoTokenizer.from_pretrained(model_name)

# model = AutoModelForMaskedLM.from_pretrained("thegoodfellas/tgf-xlm-roberta-base-pt-br")
...
for i, m in enumerate(range(0, NumberIterations)): # not using m
    ...
    # Create a classifier that uses the Roberta model
    class CustomRobertaModel(nn.Module):
        def __init__(self, n_classes, model_name):
            ...
            # self.roberta = RobertaModel.from_pretrained(model_name, return_dict=False)
    ...
    for epoch in range(epochs):
        print(f'Roberta {i}: Epoch {epoch + 1}/{epochs}')
